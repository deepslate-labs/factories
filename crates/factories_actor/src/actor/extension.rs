//! Pointer-keyed, type-erased context injected into an actor's shared state.
//!
...
impl<T> Extension<T> {
    ...
    ///
    /// Mirrors [`MessageRtti::identity`](crate::message::rtti::MessageRtti::identity) -
...
/// descriptor address.
///
/// Backed by a linear-scanned `Vec`: actors carry a handful of extensions at
...
/// Declare a `&'static` [`Extension`] descriptor, mirroring
/// [`declare_message_rtti!`](crate::declare_message_rtti).
///
/// ```ignore
...
mod tests {
    ...
    fn extension_set_is_send_and_sync() {
        ...
        // The set lives in the cross-thread-shared `SharedActorState`.
